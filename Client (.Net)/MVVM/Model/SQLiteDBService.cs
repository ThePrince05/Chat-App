    ...
    public void SaveSettings(string supabaseUrl, string supabaseApiKey, bool isDedicatedServerEnabled, string serverIp, int serverPort)
    {
        try
        {
            using (var connection = new SQLiteConnection($"Data Source={_databaseFilePath};Version=3;"))
            {
                ...
                // Update the first row (assuming row 1 is the one you want to modify)
                var command = new SQLiteCommand("UPDATE Settings SET SupabaseUrl = @SupabaseUrl, SupabaseApiKey = @SupabaseApiKey, IsDedicatedServerEnabled = @IsDedicatedServerEnabled, ServerIp = @ServerIp, ServerPort = @ServerPort WHERE rowid = 1", connection);
                command.Parameters.AddWithValue("@SupabaseUrl", supabaseUrl);
                command.Parameters.AddWithValue("@SupabaseApiKey", supabaseApiKey);
                command.Parameters.AddWithValue("@IsDedicatedServerEnabled", isDedicatedServerEnabled);
                command.Parameters.AddWithValue("@ServerIp", serverIp);
                command.Parameters.AddWithValue("@ServerPort", serverPort);

                int rowsAffected = command.ExecuteNonQuery();

                // If no rows are affected, we can insert the first row (just in case the table is empty)
                if (rowsAffected == 0)
                {
